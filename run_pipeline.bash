...
use_resnet=$(get_param "use_resnet")
...
ecg_processing_mode=$(get_param "ecg_processing_mode")
resnet_preprocessing_folder=$(get_param "resnet_preprocessing_folder")
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --ecg_processing_mode)
            if [[ -n $2 && ! $2 =~ ^-- ]]; then
                ecg_processing_mode="$2"
                shift 2
            else
                echo "Error: --ecg_processing_mode requires a non-empty argument (auto, single, batch)."
                usage
                return 1
            fi
            ;;
...
run_pipeline() {
    ...
    python main.py \
        ...
        --resnet_preprocessing_folder $resnet_preprocessing_folder
}
